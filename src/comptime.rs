...
pub fn elaborate(program: Vec<Stmt>) -> Result<Vec<Stmt>, ComptimeError> {
    ...
    elab.monomorphize(materialized)
}
...
impl<'a> Elab<'a> {
    ...
    fn generate_spec(&self, orig: &str, vals: &[CtValue]) -> Result<Stmt, ComptimeError> {
        ...
        let StmtKind::Def {
            ...
            unreachable!("specializable templates are `def`s");
        };
    ...
    fn template_decls(&self, name: &str) -> Vec<ParamDecl> {
        match &self.specializable[name].kind {
            StmtKind::Def { type_params, .. } => classify_ct_params(type_params),
            _ => unreachable!("specializable templates are `def`s"),
        }
    }
