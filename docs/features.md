...
| Borrowing | Local loans and origin-bearing cross-call references | Run | Frame/slot handles execute reborrows, free and receiver-projected returns, unions, captured indexes, and reference-valued aggregates. Static, untracked, and unsafe-any contracts lower explicitly, and `UnsafePointer(to=place)` attaches executable owner loans to pointers and pointer-storing aggregates. |
| Unsafe pointers | Provenance, arithmetic, and lifetime | Run | Stable allocation identities, element offsets, allocation bounds, aligned allocation, dangling placeholders, real deallocation, and invalid/double-free diagnostics execute in the VM. Place-origin pointers execute as frame/slot handles; their checked origins reject offsets, arithmetic, comparison, `free()`, and escapes, and only the VM value erases provenance. |
