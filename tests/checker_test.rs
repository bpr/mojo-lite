use mojo_lite::{check, TypeError, Lexer, Parser};
...
fn accepts_struct_construction_field_and_method() {
    ok(&format!("{POINT}var p: Point = Point(1, 2)\nvar s: Int = p.sum()\nvar a: Int = p.x\n"));
}
...
fn rejects_unknown_field() {
    let e = err(&format!("{POINT}var p: Point = Point(1, 2)\nvar z: Int = p.z\n"));
...
fn rejects_wrong_type_argument_count() {
    ...
    assert!(matches!(e, TypeError::WrongTypeArgCount { expected: 1, got: 2, .. }), "got {:?}", e);
}

#[test]
fn rejects_type_arguments_on_non_generic_struct() {
    let e = err("@fieldwise_init\nstruct Point:\n    var x: Int\n\nvar p: Point[Int] = Point(1)\n");
    assert!(matches!(e, TypeError::WrongTypeArgCount { expected: 0, got: 1, .. }), "got {:?}", e);
}
...
fn accepts_trait_conformance_and_bounded_call() {
    ok(&format!("{QUACK}var d: Duck = Duck(\"Donald\")\nvar s: String = make_it_quack(d)\n"));
}
...
fn rejects_argument_not_conforming_to_bound() {
    let e = err(&format!("{QUACK}@fieldwise_init\nstruct Cat:\n    var n: Int\n\nvar s: String = make_it_quack(Cat(1))\n"));
    assert!(
...
fn rejects_struct_with_mismatched_trait_method_signature() {
    ...
    let e = err("trait Quackable:\n    def quack(self) -> String:\n        ...\n\n@fieldwise_init\nstruct Duck(Quackable):\n    var name: String\n\n    def quack(self) -> Int:\n        return 1\n");
    assert!(matches!(e, TypeError::TraitMethodMismatch { .. }), "got {:?}", e);
}
...
fn rejects_bounded_type_parameter_forwarded_to_stronger_bound() {
    ...
    assert!(matches!(e, TypeError::TraitNotSatisfied { .. }), "got {:?}", e);
}
...
fn accepts_value_parameter_struct_and_self_read() {
    ok(&format!("{FIXEDBUF}var b: FixedBuffer[8] = FixedBuffer[8](0)\nvar c: Int = b.capacity()\n"));
}

#[test]
fn accepts_comptime_arithmetic_argument() {
    ok(&format!("{FIXEDBUF}var b: FixedBuffer[2 + 3 * 2] = FixedBuffer[8](0)\n"));
}
...
fn distinct_value_arguments_are_distinct_types() {
    ...
    let e = err(&format!("{FIXEDBUF}var b: FixedBuffer[5] = FixedBuffer[6](0)\n"));
    assert!(matches!(e, TypeError::TypeMismatch { .. }), "got {:?}", e);
}
...
fn rejects_uninferable_value_parameter() {
    ...
    let e = err(&format!("{FIXEDBUF}var b: FixedBuffer[4] = FixedBuffer(0)\n"));
    assert!(
...
fn rejects_non_comptime_value_argument() {
    ...
    let e = err(&format!("{FIXEDBUF}var x: Int = 3\nvar b: FixedBuffer[x] = FixedBuffer[x](0)\n"));
    assert!(matches!(e, TypeError::NotComptime(_)), "got {:?}", e);
}

#[test]
fn rejects_type_argument_for_value_parameter() {
    let e = err(&format!("{FIXEDBUF}var b: FixedBuffer[Int] = FixedBuffer[Int](0)\n"));
    assert!(matches!(e, TypeError::TypeMismatch { .. }), "got {:?}", e);
}

#[test]
fn rejects_wrong_parameter_count() {
    let e = err(&format!("{FIXEDBUF}var b: FixedBuffer[8, 9] = FixedBuffer[8, 9](0)\n"));
    assert!(matches!(e, TypeError::WrongTypeArgCount { .. }), "got {:?}", e);
}
...
fn rejects_explicit_params_on_non_generic_function() {
    let e = err("def f(x: Int) -> Int:\n    return x\n\nvar y: Int = f[Int](1)\n");
    assert!(matches!(e, TypeError::WrongTypeArgCount { expected: 0, .. }), "got {:?}", e);
}
...
fn accepts_simd_construction_ops_and_index() {
    ok("var v: SIMD[DType.int32, 4] = SIMD[DType.int32, 4](1, 2, 3, 4)\nvar w: SIMD[DType.int32, 4] = v + v * v\nvar lane: Int32 = w[0]\n");
}
...
fn accepts_simd_comparison_mask() {
    ok("var v: SIMD[DType.float32, 4] = SIMD[DType.float32, 4](1.0, 2.0, 3.0, 4.0)\nvar m: SIMD[DType.bool, 4] = v < v\n");
}
...
fn rejects_wrong_simd_element_count() {
    ...
    assert!(matches!(e, TypeError::SimdArity { width: 4, got: 2 }), "got {:?}", e);
}

#[test]
fn rejects_mixed_dtype_operands() {
    let e = err("var a: SIMD[DType.int32, 4] = SIMD[DType.int32, 4](1, 2, 3, 4)\nvar b: SIMD[DType.int64, 4] = SIMD[DType.int64, 4](1, 2, 3, 4)\nvar c: SIMD[DType.int32, 4] = a + b\n");
    assert!(matches!(e, TypeError::BadOperator { .. }), "got {:?}", e);
}
...
fn rejects_raising_a_non_error() {
    let e = err("raise 5\n");
    assert!(matches!(&e, TypeError::TypeMismatch { context, .. } if context == "raise"), "got {:?}", e);
}
...
fn rejects_builtin_arity_mismatch() {
    assert!(matches!(err("var a: Int = abs(1, 2)\n"), TypeError::ArityMismatch { .. }));
    assert!(matches!(err("var a: Int = min(1)\n"), TypeError::ArityMismatch { .. }));
}
...
fn rejects_empty_inferred_list() {
    ...
    assert!(matches!(e, TypeError::CannotInferTypeParam { .. }), "got {:?}", e);
}
...
fn rejects_remove_on_non_equatable_elements() {
    let e = err("@fieldwise_init\nstruct P:\n    var x: Int\n\nvar ps: List[P] = [P(1)]\nps.remove(P(1))\n");
    assert!(matches!(&e, TypeError::TypeMismatch { context, .. } if context == "'remove'"), "got {:?}", e);
}
...
fn accepts_field_and_nested_place_writes() {
    ok(&format!("{MUTPT}var p: Point = Point(1, 2)\np.x = 10\np.y = p.x + 1\n"));
    ok("@fieldwise_init\nstruct In:\n    var v: Int\n\n@fieldwise_init\nstruct Out:\n    var inner: In\n\nvar o: Out = Out(In(5))\no.inner.v = 100\n");
    ok(&format!("{MUTPT}var ps: List[Point] = [Point(1, 1)]\nps[0].x = 9\n"));
}
...
fn rejects_write_to_a_call_result() {
    let e = err(&format!("{MUTPT}def mk() -> Point:\n    return Point(0, 0)\n\nmk().x = 5\n"));
    assert!(matches!(e, TypeError::InvalidAssignTarget(_)), "got {:?}", e);
}
...
fn rejects_augmented_assignment_that_changes_type() {
    ...
    assert!(matches!(&e, TypeError::TypeMismatch { context, .. } if context == "augmented assignment"), "got {:?}", e);
}

#[test]
fn rejects_augmented_assignment_bad_operands() {
    assert!(matches!(err("var i: Int = 1\ni += True\n"), TypeError::BadOperator { .. }));
...
fn rejects_inferred_var_of_range() {
    ...
    assert!(matches!(&e, TypeError::TypeMismatch { found, .. } if found == "range"), "got {:?}", e);
}
...
fn accepts_tuple_annotated_inferred_and_indexed() {
    ok("var t: Tuple[Int, Float64, String] = (1, 2.5, \"hi\")\nvar a: Int = t[0]\nvar b: Float64 = t[1]\nvar c: String = t[2]\n");
    ok("var u = (10, 20)\nvar first: Int = u[0]\n");
    ok("def stats() -> Tuple[Int, Int]:\n    return (512, 4)\n\nvar s = stats()\nvar p: Int = s[0]\n");
}
...
fn rejects_runtime_tuple_index() {
    ...
    assert!(matches!(&e, TypeError::TypeMismatch { context, .. } if context == "tuple index"), "got {:?}", e);
}
...
fn flags_advanced_parameter_forms_as_unsupported() {
    for src in [
        ...
        assert!(matches!(err(src), TypeError::Unsupported(_)), "expected Unsupported for: {src}");
    }
}
...
fn accepts_variadic_args() {
    ...
    ok("def sum(*values: Int) -> Int:\n    var t: Int = 0\n    for v in values:\n        t = t + v\n    return t\n\nvar a: Int = sum()\nvar b: Int = sum(1, 2, 3)\n");
    // Regular params before the variadic.
    ok("def tag(label: String, *nums: Int) -> Int:\n    return len(nums)\n\nvar a: Int = tag(\"x\", 1, 2)\n");
...
fn accepts_default_argument_values() {
    ...
    ok("def my_pow(base: Int, exp: Int = 2) -> Int:\n    return base ** exp\n\nvar a: Int = my_pow(3)\nvar b: Int = my_pow(3, 3)\n");
}
...
fn accepts_keyword_arguments_and_reports_mismatches() {
    ...
    for src in [
        ...
        assert!(matches!(err(src), TypeError::BadCall { .. }), "expected BadCall for: {src}");
    }
...
fn accepts_numeric_bases_and_string_forms_but_flags_tstrings() {
    ...
    assert!(matches!(err("var s: String = t\"x={y}\"\n"), TypeError::Unsupported(_)));
}
