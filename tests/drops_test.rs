...
fn transferred_value_is_dropped_once_at_destination() {
    ...
    let src = format!("{RES}def main():\n    var a: Res = Res(5)\n    var b: Res = a^\n    print(b.id)\n");
    let out = vm(&src);
    assert_eq!(out.matches("del 5").count(), 1, "moved value dropped exactly once");
    assert_eq!(out, "del 5\n5\n");
}
...
fn partially_moved_field_is_dropped_once_at_its_new_owner() {
    ...
    let out = vm(src);
    assert_eq!(out.matches("del 1").count(), 1, "moved field dropped exactly once");
    assert_eq!(out.matches("del 2").count(), 1, "retained field dropped once");
...
fn owned_parameter_is_dropped_by_the_callee() {
    ...
    let src = format!("{RES}def consume(owned t: Res):\n    print(\"consuming\", t.id)\n\ndef main():\n    var a: Res = Res(1)\n    consume(a^)\n    print(\"done\")\n");
    let out = vm(&src);
...
fn borrowed_parameter_is_not_dropped_by_the_callee() {
    ...
    let src = format!("{RES}def peek(t: Res) -> Int:\n    return t.id\n\ndef main():\n    var a: Res = Res(2)\n    var n: Int = peek(a)\n    print(n)\n");
    let out = vm(&src);
    assert_eq!(out.matches("del 2").count(), 1, "destroyed exactly once (by the caller)");
}
...
fn del_runs_when_a_try_body_is_left() {
    ...
    let raising = format!("{RES}def main():\n    try:\n        var r: Res = Res(1)\n        print(\"have\", r.id)\n        raise \"boom\"\n    except e:\n        print(\"caught\")\n    print(\"done\")\n");
    assert_eq!(vm(&raising), "have 1\ndel 1\ncaught\ndone\n");

    let normal = format!("{RES}def main():\n    try:\n        var r: Res = Res(2)\n        print(\"have\", r.id)\n    except e:\n        print(\"caught\")\n    print(\"done\")\n");
...
fn break_crossing_try_drops_body_local_and_outer_loop_local() {
    ...
    assert_eq!(out.matches("drop 21").count(), 1, "body-local of the break iteration dropped once");
    assert_eq!(out.matches("drop 11").count(), 1, "outer loop-local of the break iteration dropped once");
    assert!(out.ends_with("fin 1\ndone\n"), "finally reads the loop var, then done:\n{out}");
